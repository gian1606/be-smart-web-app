...
export default function UserManagement() {
  ...
  function openEdit(item) {
    setEditItem(item);
    setForm({ name: item.name, collectorId: item.collectorId, assignedTruck: item.assignedTruck, zone: item.zone, schedule: item.schedule, tempPassword: "" });
  ...
  function handleSave() {
    ...
    if (editItem) {
      setCollectors((prev) =>
        prev.map((c) =>
          c.id === editItem.id
            ? { ...c, name: form.name, collectorId: form.collectorId, assignedTruck: form.assignedTruck, zone: form.zone, schedule: form.schedule }
            : c
        )
      );
    } else {
  ...
  function handleDeactivate(id) {
    setCollectors((prev) =>
      prev.map((c) => c.id === id ? { ...c, status: c.status === "active" ? "off_duty" : "active" } : c)
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-text-secondary mt-0.5" style={{ fontSize: 14 }}>
            Collectors &amp; Drivers — {CLUSTER_INFO.label}
          </p>
        </div>
        <button
          onClick={openAdd}
              ...
              <tr key={c.id} style={{ background: i % 2 === 0 ? "#fff" : "#FAFAFA", borderBottom: "1px solid #F3F4F6" }}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <button onClick={() => openEdit(c)} className="p-1.5 rounded-lg hover:bg-gray-100 transition-colors" title="Edit">
                      <Pencil size={15} color="#6B7280" />
                    </button>
                    <button onClick={() => handleDeactivate(c.id)} className="p-1.5 rounded-lg hover:bg-gray-100 transition-colors" title="Toggle status">
                      <UserX size={15} color={c.status === "active" || c.status === "on_route" ? "#D32F2F" : "#2E7D32"} />
                    </button>
                  </div>
                </td>
              </tr>
      ...
      >
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr style={{ background: "#F9FAFB", borderBottom: "1px solid #E5E7EB" }}>
                <th className="text-left font-semibold uppercase tracking-wide px-4 py-3" style={{ fontSize: 12, color: "#6B7280", minWidth: 140 }}>
                  Collector
                </th>
                {DAYS.map((d) => (
                  <th key={d} className="text-center font-semibold uppercase tracking-wide px-3 py-3" style={{ fontSize: 12, color: "#6B7280", minWidth: 80 }}>
                    {d}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {collectors.map((c, i) => {
                const scheduledDays = c.schedule.split(",")[0].trim(); // e.g. "Mon–Fri"
                return (
                  <tr key={c.id} style={{ background: i % 2 === 0 ? "#fff" : "#FAFAFA", borderBottom: "1px solid #F3F4F6" }}>
                    <td className="px-4 py-3 font-semibold text-text-primary" style={{ fontSize: 13 }}>{c.name}</td>
                    {DAYS.map((d) => {
                      ...
                      return (
                        <td key={d} className="px-3 py-3 text-center">
                          {isScheduled ? (
                            <span
                              className="inline-block rounded-md px-2 py-1 font-medium"
                            ...
                            <span
                              className="inline-block rounded-md px-2 py-1 font-medium"
                              style={{ fontSize: 11, background: "#F3F4F6", color: "#9CA3AF" }}
                            >
                              Rest
                            </span>
                          )}
      ...
      <Modal
        ...
        title={editItem ? "Edit Collector" : "Add Collector"}
        footer={
          <>
            ...
            <button onClick={handleSave} className="rounded-lg px-5 py-2 font-semibold text-white hover:opacity-90 transition-opacity" style={{ fontSize: 14, background: "#2E7D32" }}>
              {editItem ? "Save Changes" : "Add Collector"}
            </button>
          </>
        }
      >
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <select
              ...
              onChange={(e) => setForm((p) => ({ ...p, assignedTruck: e.target.value }))}
              className="rounded-lg px-3 py-2.5 outline-none"
              style={{ fontSize: 14, border: formErrors.assignedTruck ? "1.5px solid #D32F2F" : "1.5px solid #E5E7EB", background: "#F9FAFB", color: "#1A1A1A" }}
            >
              <option value="">Select a truck…</option>
              {TRUCKS_LIST.map((t) => <option key={t} value={t}>{t}</option>)}
            </select>
            {formErrors.assignedTruck && <span style={{ fontSize: 12, color: "#D32F2F" }}>{formErrors.assignedTruck}</span>}
          </div>
          <div className="flex flex-col gap-1">
