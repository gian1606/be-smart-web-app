...
export default function UserManagement() {
  ...
  function openAdd() {
    setEditUser(null); setForm(EMPTY_FORM); setFormErrors({}); setModalOpen(true);
  }
  function openEdit(user) {
    setEditUser(user);
    setForm({ name: user.name, email: user.email, assignedCluster: user.assignedCluster, tempPassword: "" });
  ...
  function handleSave() {
    ...
    if (editUser) {
      ...
      setUsers((prev) => [
        ...prev,
        { id: `u${Date.now()}`, name: form.name, email: form.email, assignedCluster: form.assignedCluster, role: "cluster_admin", status: "active", lastLogin: null },
      ]);
    }
    setModalOpen(false);
  }
    ...
    <div className="flex flex-col gap-6">
              ...
              <tr key={u.id} style={{ background: i % 2 === 0 ? "#fff" : "#FAFAFA", borderBottom: "1px solid #F3F4F6" }}>
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <button onClick={() => openEdit(u)} className="p-1.5 rounded-lg hover:bg-gray-100 transition-colors" title="Edit">
                      <Pencil size={15} color="#6B7280" />
                    </button>
                    <button onClick={() => handleDeactivate(u.id)} className="p-1.5 rounded-lg hover:bg-gray-100 transition-colors" title={u.status === "active" ? "Deactivate" : "Activate"}>
      ...
      >
        <div className="flex flex-col gap-4">
          <FormField label="Full Name" value={form.name} onChange={(v) => setForm((p) => ({ ...p, name: v }))} error={formErrors.name} placeholder="e.g. Juan dela Cruz" />
          <FormField label="Email Address" type="email" value={form.email} onChange={(v) => setForm((p) => ({ ...p, email: v }))} error={formErrors.email} placeholder="juan.delacruz@besmart.gov.ph" />
          <div className="flex flex-col gap-1">
            ...
            <select
              ...
              style={{ fontSize: 14, border: formErrors.assignedCluster ? "1.5px solid #D32F2F" : "1.5px solid #E5E7EB", background: "#F9FAFB", color: "#1A1A1A" }}
            >
              <option value="">Select a cluster…</option>
              {CLUSTERS.map((c) => <option key={c.id} value={c.id}>{c.label}</option>)}
            </select>
            {formErrors.assignedCluster && <span style={{ fontSize: 12, color: "#D32F2F" }}>{formErrors.assignedCluster}</span>}
          </div>
          <div className="flex flex-col gap-1">
            <label className="font-medium text-text-primary" style={{ fontSize: 13 }}>Role</label>
            <input value="Cluster Admin" disabled className="rounded-lg px-3 py-2.5" style={{ fontSize: 14, border: "1.5px solid #E5E7EB", background: "#F3F4F6", color: "#9CA3AF" }} />
          </div>
          {!editUser && (
